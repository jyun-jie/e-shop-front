...
<script setup>
    ...
    import {goGetOrderByUnchecked , goCheckSelectedOrders} from '@/api/token.js'
    ...
    onMounted(async()=>{
        ...
        //有問題
        const data = await goGetOrderByUnchecked('1',config);
    ...
    if(oldButtonId.value !== buttonId){
      ...
      const data = await goGetOrderByUnchecked(buttonId,config)
      salesOrder.value = data.data
    ...
    const CheckSelectedOrders = () => {
      ...
      const data = goCheckSelectedOrders(selectedOrders.value,config)
      // 出貨後清空選擇
